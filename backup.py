...
def send_telegram_message(message):
    ...
    if not bot_token or not chat_id:
        print("TELEGRAM_BOT_TOKEN и TELEGRAM_CHAT_ID не заданы.")
        return
...
def main():
    try:
        ...
        required_vars = {
            'PG_HOST': pg_host, 'PG_USER': pg_user, 'PG_DATABASE': pg_database, 
            'PG_PASSWORD': pg_password, 'DUMP_FILE_NAME': dump_file, 
            'S3_PREFIX': prefix, 'BUCKET_NAME': bucket_name
        }
        ...
        if os.path.exists(dump_file) and os.path.getsize(dump_file) > 0:
            ...
            destination_filename = f'{prefix}/{os.path.splitext(dump_file)[0]}_{timestamp}UTC.sql'
            upload_to_s3(dump_file, destination_filename, bucket_name)
            send_telegram_message(f"✅ Бэкап {destination_filename} успешно загружен в R2.")
        else:
            send_telegram_message(f"❌ Ошибка: файл дампа {dump_file} пустой или не создан.")
...
def upload_to_s3(source_path, destination_filename, bucket_name):
    ...
    try:
        ...
        # Загрузка файла с префиксом (папкой)
        s3.upload_file(source_path, bucket_name, destination_filename)
        print(f"Файл успешно загружен в {bucket_name}/{destination_filename}")
